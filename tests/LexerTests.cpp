...
{
    LoxToken{ TokenType::CommentBegin, 1, 0 },
    LoxToken{ TokenType::CommentString, 1, 3, commentStr },
    LoxToken{ TokenType::Print, 2, 0 },
    LoxToken{ TokenType::StringLiteral, 2, 7, literalStr },
    LoxToken{ TokenType::Semicolon, 2, 21 },
...
{
    LoxToken{ TokenType::Var, 1, 0 },
    LoxToken{ TokenType::Identifier, 1, 4, TestValue0_ },
    LoxToken{ TokenType::Equal, 1, 16 },
    LoxToken{ TokenType::NumberLiteral, 1, 18, 1.234f },
    LoxToken{ TokenType::Semicolon, 1, 23 },
    LoxToken{ TokenType::Var, 2, 0 },
    LoxToken{ TokenType::Identifier, 2, 4, Test_Value_2 },
    LoxToken{ TokenType::Equal, 2, 17 },
    LoxToken{ TokenType::StringLiteral, 2, 20, TestEXCLAIMS },
    LoxToken{ TokenType::Semicolon, 2, 26 },
    LoxToken{ TokenType::EndOfFile, 3, 0 }
};

const char* BrokenErrorHandlingTestSource =
R"(
...
std::string_view RunBasicLexerTests()
{
        ...
        firstMismatchIter.second == tokens.end())
    {
        ...
        std::string resultTokens = Helpers::GetLoxTokensString(tokens.size(), &tokens[0]);
    ...
    else
    {
        ...
        auto testFailure =
            Helpers::HandleTestFailure(*firstMismatchIter.first, test1ArrayLen, *firstMismatchIter.second, test1TokensLen, testFailPos);
        std::cout << Helpers::ErrorCodeMessage(testFailure.errorCode) << ',';
    ...
    else
    {
        ...
        auto testFailure =
            Helpers::HandleTestFailure(*secondMismatchIter.first, test2ArrayLen, *secondMismatchIter.second, test2TokensLen, testFailPos);
        std::cout << Helpers::ErrorCodeMessage(testFailure.errorCode) << ',';
        std::cout << testFailure.message << '\n';
    ...
    try
    {
        result = lexer.ParseScript(BrokenErrorHandlingTestSource);
    }
